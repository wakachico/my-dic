...
### Mydic（自分の辞書）の作成
    基本的要素である「英単語」「和訳」に追加して、自分なりのテキストを付属して保存ができます。テキストにはジャンルをつけれるため、例文や語呂、アニメや歌詞などお気に入りのテキストをつけて登録しよう！自分のための自分だけの世界に１つだけの辞書を作成しよう♪
---
### ワードの公開
    お気に入りのテキストを登録できたら他ユーザーに公開してみんなでシェアしよう♪
    公開ワードはグー（グッド）ができるぞ！
    また、他ユーザーが公開しているワードを自分の辞書に登録できる「採用機能」も搭載しています。
    自分に合ったテキストを自分の辞書に登録し辞書を分厚くしていこう♪
---
### 確認テスト
    Mydicに登録したワードを「和訳テスト」「英単語テスト」の２タイプでテストする事ができます。
    辞書に登録したワードを覚えた！と思ったらテスト機能を使って確認してみよう♪
...
# 制作背景
  - 制作のきっかけとしてはプログラミングを学習し始めて英語力に課題を感じたため何か学習をサポートしてくれるアプリを制作したいと感じたことです。
  - アプリの対象者としては「プログラミング初学者」「中高生」です。
  - プログラミング初学者はテキストにメソッドの使用例などを登録できたら学習ペースアップが狙えます。
  - 中高生に対しては、特に英語教科の勉強に消極的な人で尚且つSNSなどのツールをよく利用している人がペルソナです。学校の学習は全国で大体同じペースで行われるため、単元で学習した単語をどのようなテキストをつけて登録するのかをお互いに楽しみにしながらアプリ利用ができるのではないかと考えてます。
